...
import { COMPLIANCE_SCORES } from './compliance/mockData';
...
export function ComplianceApprovals({ searchQuery: externalSearch = "" }: { searchQuery?: string }) {
  ...
  const handleComplianceBulkAction = (action: 'approve' | 'reject') => {
      if (selectedIds.size === 0) return;
      if (action === 'approve') {
          setRequests(prev => prev.map(r => selectedIds.has(r.id) ? { ...r, status: 'Approved' } : r));
          toast.success(`Approved ${selectedIds.size} requests locally`);
      } else {
          setRequests(prev => prev.map(r => selectedIds.has(r.id) ? { ...r, status: 'Rejected' } : r));
          toast.success(`Rejected ${selectedIds.size} requests locally`);
      }
      setSelectedIds(new Set());
  };
